...
class ActivatorEmpty {
  ...
  back(A) {
    if (Array.isArray(A)) return A.map((a) => 1);
    else return A.apply((a) => 1);
  }
}
...
class ActivatorTanh {
  ...
  forward(A) {
    if (Array.isArray(A)) return A.map((a) => this.tanh(a));
    else return A.apply((a) => this.tanh(a));
  }
  back(A) {
    if (Array.isArray(A)) return A.map((a) => 1 - Math.pow(this.tanh(a), 2));
    else return A.apply((a) => 1 - Math.pow(this.tanh(a), 2));
  }
}
class ActivatorSigmoid {
  ...
  forward(A) {
    if (Array.isArray(A))
      return A.map((a) => {
        if (Array.isArray(a)) return a.map((b) => 1 / (1 + Math.exp(-b)));
        else return 1 / (1 + Math.exp(-a));
      });
    else return A.apply((a) => 1 / (1 + Math.exp(-a)));
  }
  back(A) {
    if (Array.isArray(A))
      return A.map((a) => {
...
class ActivatorReLU {
  ...
  forward(A) {
    if (Array.isArray(A))
      return A.map((a) => {
        if (Array.isArray(a)) return a.map((b) => Math.max(b, 0));
        else return Math.max(a, 0);
      });
    else return A.apply((a) => Math.max(a, 0));
  }
  back(A) {
    if (Array.isArray(A))
      return A.map((a) => {
        if (Array.isArray(a)) return a.map((b) => (b > 0 ? 1 : 0));
        else return a > 0 ? 1 : 0;
      });
    else return A.apply((a) => (a > 0 ? 1 : 0));
  }
}
// Activators -- end
...
class Layer {
  ...
  setData(A) {
    if (Array.isArray(A)) this.matrix = new Matrix(1, this.size, ...A);
    else this.matrix.elements = A.elements;
  }
